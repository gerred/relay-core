...
func ConfigureKnativeService(ctx context.Context, s *KnativeService, wtd *WebhookTriggerDeps) error {
	...
	if len(wtd.WebhookTrigger.Object.Spec.Input) > 0 {
		...
		if wtd.Tenant.Object.Spec.ToolInjection.VolumeClaimTemplate != nil {
			ep, err := image.ImageEntrypoint(i, []string{wtd.WebhookTrigger.Object.Spec.Command}, wtd.WebhookTrigger.Object.Spec.Args)
			if err != nil {
				return err
			}

			container.Command = []string{path.Join(model.EntrypointVolumeMountPath, model.EntrypointCommand)}
			container.Args = ep
		} else {
	...
	if wtd.Tenant.Object.Spec.ToolInjection.VolumeClaimTemplate != nil {
		Annotate(&template.ObjectMeta, model.RelayControllerVolumeClaimAnnotation, wtd.Tenant.Object.GetName()+model.EntrypointVolumeClaimSuffixReadOnlyMany)
	}
